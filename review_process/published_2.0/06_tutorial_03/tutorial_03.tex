...
Even if the dissociation path is determined during the simulation, it is often assumed that it is linear, or that only a single dissociation path exists. 
...
The preparation of the topology, coordinates, energy minimization and equilibration of the system are very similar to the tutorial above. 
...
Together with \texttt{NSTLIM = 500000} (from the \texttt{STEP} block) and an integration time step of 0.002~ps, this results in a simulation of 1~ns where $\lambda$ is continuously changed from 0 to 1. 
...
The \texttt{DISTANCEFIELD} block is largely unchanged, apart from \texttt{NTWDF} because we will write out the DF data more often. You will also find an additional block:
\begin{lstlisting}
REPLICA
# NRET
  1
...
END
\end{lstlisting}
Because we will perform Hamiltonian-replica-exchange and not temperature replica exchange, the number of replica exchange temperatures is set to \texttt{NRET=1}. 
...
\texttt{CONT=1}, as we start from multiple configuration files, instead of a single configuration.
...
The program will find the $\lambda$ values of the replicas, look for the DF restraint in the \texttt{PERTDFRESSPEC} block in the distance restraints file and determine the restraining distances for each of the replicas from this information.
It will then search for the frame in the slow-growth simulation which has a DF distance that is closest to the restraining distance of this replica.
...
$ mk_script @f mk_script.arg
\end{lstlisting}
The HREMD simulations are split into multiple runs, in order to prevent an extremely long single simulation. 
Go to the first directory, \texttt{run1} and submit \texttt{HREMD\_1.run} to a computer cluster, preferably with the same number of CPUs as we have replicas, which would be 24 in the prepared example. 
...
\end{lstlisting}
Here, you need to replace [nr\_runs] with the number of runs that have already been performed and [nr\_trials] with the number of exchange trials per run, which was set to 100 in the example files.
...
\end{lstlisting}
The resulting \texttt{rep\_change.out} file can be opened with \linebreak Xmgrace. An example of such a file is shown in Figure~\ref{rep_change}.

\begin{figure}[t]
\centering
\includegraphics[scale=.32]{../06_tutorial_03/figures/rep_change}
...
The former option may be quicker to set up, but requires longer simulation time because of the additional replica. 
...
First, we have to write out the measured DF distances for each replica. Then we calculate the distributions of these and on this data we can perform the weighted histogram analysis method (WHAM) which will result in the FEC.
...
Especially the boundaries should be adjusted when working on different systems. 
...
The file \texttt{wham\_FEC\_200bins\_ 5000iter\_min0.dat} now contains the final FEC, which is shown in Fig.~\ref{FEC}. As you can see, the curve is not completely flat at larger distances, but is rather noisy. Ideally, you would have to change the spacing of the replicas, add more replicas in the unbound range, or lower the maximum distance which you restrain such that the replicas are placed more densely in the unbound range. 
...
\begin{figure}[t]
\centering
\includegraphics[scale=.32]{../06_tutorial_03/figures/FEC}
...
Note that in \cite{deRuiter_2013}, $F(r)$ is referred to as $\Delta G_{\text{WHAM}}(r)$. The expression (Eq. 12) used in that paper to calculate the binding free energy is obtained when shifting $F(r)$ to become $\hat{F}(r) = F(r) + C$, such that $\int_{\text{b}} \mathrm{d}r\: e^{-\hat{F}(r)/k_{\text{B}} T}$ equals 1. This is achieved for 
\begin{equation}
  \begin{aligned}
    C = k_{\text{B}}T \ln \int_{\text{b}} \mathrm{d}r\: e^{-F(r)/k_{\text{B}} T}
  \end{aligned}
\end{equation}
Note that the minus sign in Eq. 12 of ref~\cite{deRuiter_2013} should actually be a plus sign.
...
\paragraph{Closing of the binding site}
In some cases the binding site closes up, preventing a DF path from the active site to the solvent. As a result, all grid points outside the protein will get the additional protein penalty as defined with the parameter \texttt{PROTEINOFFSET}. If the site remains buried for some time, this becomes problematic especially if the ligand is unbound. Since all grid points outside the protein have the large \texttt{PROTEINOFFSET}, the DF distance basically becomes a radial distance again, and ligands could be forced to move through the protein instead of around it.

This issue is easily detected by a large jump in the DF distance as soon as the binding site closes.

A proper definition of the zero-distance point helps prevent the closure of the binding site. This point is typically the center of geometry of several atoms in the binding site. If your binding is closing during the simulation, try the following tips for the definition of the zero-distance point:

\begin{enumerate}
   \item[-] Choose more stable atoms (e.g. backbone atoms) to ensure that the zero-distance point remains fixed and unaffected by side chain movements.
   \item[-] Use more atoms to define the zero-distance point to distribute the restraint force and reduce distortion of the active site.
\end{enumerate}

...
In some cases it can happen that the binding site closes up, such thatthere is no DF path possible from the active site to the solvent. In this case all grid points outside of the protein will get the additional protein penalty as defined with the parameter \texttt{PROTEINOFFSET}. If the site remains buried for some time, this becomes problematic especially if the ligand is unbound. Since all grid points outside the protein have the large \texttt{PROTEINOFFSET}, the DF distance basically becomes a radial distance again, and ligands could be forced to move through the protein instead of around it.

\subparagraph{Indication}
The problem can be easily discovered by monitoring the DF distances during the simulation. As soon as the binding site closes, there will be a large jump in the DF distance, corresponding to the \texttt{PROTEINOFFSET} penalty.

\subparagraph{Recommendation}
One of the most important steps to prevent the closure of the binding site is the proper choice of the zero distance point. This point is typically the center of geometry of several atoms in the binding site. If you find that your binding is closing during the simulation, try the following tips for the definition of the zero distance point:

\begin{enumerate}
   \item[-] choose more stable atoms (e.g. backbone atoms)
   \item[-] use more atoms to define the zero distance point
\end{enumerate}

The first point ensures that the zero distance point is stably defined and does not move through the active site due to e.g. side chain movements. The second point ensures that the force caused by the applied distance restraint is distributed over more atoms, making it less likely to cause distortions of the active site.

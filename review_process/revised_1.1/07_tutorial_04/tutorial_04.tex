...
Gaussian accelerated MD is a recently developed technique that allows to increase the sampling without the need of \textit{a priori} knowledge of the cause of the energy barriers. GaMD works by adding a boosting potential-energy term that flattens the energy landscape.
...
The next lines assigns each group of interactions to their corresponding acceleration group where \texttt{AGROUP\_1} and \texttt{AGROUP\_2}  are the indexes of the atom groups defined in the previous block and \texttt{ACCELGROUP} is the index for the acceleration potential-energy term. 
...
A common source of error in GaMD simulations is an undesired or erroneous selection of parameters for acceleration and interaction groups. If the selected inputs are incompatible, the program will terminate with an error. More concerning, however, is the possibility that the simulation completes successfully while producing accelerations that deviate from those intended by the user.
...
\begin{enumerate}
  ...
  \end{aligned}
\end{equation}
  sets of interaction pairs where $n$ is the number of interactions groups.

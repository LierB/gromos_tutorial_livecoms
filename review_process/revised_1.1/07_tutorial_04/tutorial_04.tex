...
A common source of error in GaMD simulations is an undesired or erroneous selection of parameters for acceleration and interaction groups. In the best case scenario, the selected inputs would be incompatible and will crash. In the worst case, the simulation will run without errors but the resulting acceleration may not match the ones desired by the user.
Because of this, it is recommended to check the following:
...
A solution for this problem is to decrease the amount of acceleration used on the system or to decrease the integration time step to 1 fs for a few steps to allow the system to stabilize.

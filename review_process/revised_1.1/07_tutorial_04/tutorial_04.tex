...
The most common error in setting up GaMD simulations is creating an undesired or erroneous selection of parameters for the accelerations and interaction groups. In the best case scenario, the selected inputs would be incompatible and will crash. In the worst case, the simulation will run without errors but the resulting acceleration may not match the ones desired by the user.
...
The second most common error when performing GaMD simulations is SHAKE errors. Since GaMD pumps energy into the system, although rarely, this can lead to the system blowing up. A solution for this problem is to decrease the amount of acceleration used on the system or to decrease the integration time step to 1 fs for a few steps to allow the system to stabilize.
